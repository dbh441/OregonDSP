// Copyright (c) 2011, 2021  Deschutes Signal Processing LLC
...
public class AnalogButterworth extends AnalogPrototype {
	...
	public static void main( String[] args ) {
	  ...
	  AnalogPrototype   A = B.lptobr( 2*Math.PI*2.0, 2*Math.PI*3.0 );
	  ...
	  PrintStream ps;
	  try {
		ps = new PrintStream( new FileOutputStream( "G:/DATA/Test/AnalogButterworthResponse.m" ) );
		ps.print( "R = [ " );
		for ( int i = 0;  i < 200;  i++ ) {
			ps.println( tmp[i] + "; ..." );
		}
	  ...
	  try {
	     ps = new PrintStream( new FileOutputStream( "G:/DATA/Test/AnalogButterworthGroupDelay.m" ) );
	     ps.print( "gd = [ " );
	     for ( int i = 0;  i < 200;  i++ ) {
	       ps.println( gd[i] + "; ..." );
	     }
