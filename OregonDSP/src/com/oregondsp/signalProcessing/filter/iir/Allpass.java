...
public class Allpass {
  ...
  /** boolean indicating a degenerate case:  whole sample delay. */
  ...
  /**
   * Instantiates a new degenerate allpass representing a whole sample delay.
   * 
   * @param wholeDelay  int containing the whole delay.
   * @param degenerate  indicates degenerate allpass
   */
  public Allpass ( int wholeDelay, boolean degenerate ) {
  ...
  public float filter( float x ) {
    ...
    if ( degenerate ) {
      ...
      for ( int i = order;  i > 0;  i-- ) state[i] = state[i-1];
	  state[ 0 ] = x;
